...
#define Trace_Interrupt    0x0008 //Traces command following a Ctrl+C interrupt.
#define Trace_Recovery     0x0040 //Allows stepping through recovery files.
#define Trace_DumpSeq      0x0080 //Dumps command-sequence.
...
//Any of these define a trace point
#define Trace_AnyTracePoint (Trace_AllCommands | Trace_CommandLines | Trace_Failures)
...
struct Buf {
  ...
  int CurrentChr;                 //The current character offset - strictly, in a unicode environment, it's the current byte.
...
#endif
//  int HoldChan;                   //FileDesc associated with file read with -hold qualifier.
...
#define ConsoleCommand 4          //The command was read from the console, from an -asConsole script or frmo an -init=... sequence.
#define DebuggerCommand 5         //The command was entered to the debugger.
...
char * TransCtrlChrs(char * , int);
char * GetBufferPath(struct Buf * );
...
void JotAddString(char *, int);
void JotSetAttr(int);
void JotClrToEndOfSlice(int, int, int, int);
void JotClearToEOL();
void JotAddBoundedString(char *, int, int, int *, int *, int, short unsigned int, short unsigned int);
...
void ExpandDeferredString(struct Buf *, char *, int);
...
static int s_LastLineNumber;                          //Used to detect when a compiled command came from a new line of source code.
...
static int s_Failure;                                 //An ordinary command failure has occured unwinds to else clause or exits current script 
...
static int s_ChrNo;                                   //Real character position (ie. ignoring LeftOffset) of next character to be written by WriteString()
...
static int s_Verbose = 3;
static char s_TableSeparator = '\t';
static int s_CommandMode = 0;
static int s_StackSize = 100;
static int s_StackPtr;
static int s_CaseSensitivity;
static struct Buf *s_BufferChain;
static struct Buf *s_CommandBuf;
static struct Com *s_CurrentCommand = NULL;
static int s_MaxConsoleLines = 0;                    //Sets maximum size of console area - including lines filtched from lowest windows.
...
void InitializeEditor(int ArgC, char *ArgV[])
  ...
  s_Attrs[Attr_CurrChr] =    ( FOREGROUND_INTENSITY | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE  | BACKGROUND_RED );
        ...
        case 'I': // -init=<JOT commands>  - Initialization before starting interactive session.
          ...
          for ( ; ; ) { //Identify any further args that form part of the init string.
            ...
            if ( (s_InitCommands->CurrentRec->text[ArgLength] == '\\') && (++Arg <= ArgC) ) {
              ...
              s_InitCommands->CurrentChr = ArgLength+1; }
            else
              break;
            EqChr = 0; }
        ...
        case 'S': // 'S' commands switch.
          ...
          case 'T':
            if (toupper(ArgV[Arg][4]) == 'C') //-STACksize=<n> - set the operand stack size.
              ...
              else { //A -startup pathname specified.
                free(s_StartupFileName);
  ...
  if (getenv("JOT_JOURNAL")) //Journalling requested via the JOT_JOURNAL env.
    s_JournalPath = "";
  if (s_JournalPath) {
    char FullPath[StringMaxChr], Actual[StringMaxChr], *NameElem;
    if ( ! FileName)
  ...
  if (s_NewFile) { //Create a new file.
    ...
    if (s_StreamIn) { // Reading from stdin stream.
      ...
      if (s_JournalHand) {
        ...
        UpdateJournal(Temp, NULL); }
      if ( ! FileDesc)
        Disaster("Can't open file \"%s\"", FileName);
#if defined(VC)
  ...
  s_CommandBuf->CurrentChr = 0;
  s_FirstWindow = NULL;
  s_CaseSensitivity = FALSE;
  JotSetAttr(Attr_Normal);
  
  s_TraceMode = 0;
  s_DefaultTraceMode = Trace_Stack | Trace_Print | Trace_Break | Trace_AllCommands;
  s_Failure = FALSE;
...
#endif
    if (s_Failure) {
      if ( s_asConsole && (s_Interrupt || s_Failure == (char)EOF) ) { //s_Failure is only set to EOf at the end of an -asConsole script.
        s_Failure = FALSE;
        break; }
      else
        continue; }
  ...
  for ( ; ; ) { // Command-sequence loop.
    ...
    case 'H': { //H - Here command.
      ...
      if ( ! CheckBufferKey(Chr)) {
        ...
        continue; }
      //N.B. the * arg cannot be used with a repeat count - it would be useless anyway since * has to leave the CurrentChr at the start of the insert.
    ...
    case 'O': {  //O - stack-Operation command.
      ...
      case 'L': { //OL - OL<nnn>[.<mmm>] - Literal, integer of floating, number to be pushed onto stack.
        int Chr, Chrs = 0;
        int InitialCharacter = CommandBuf->CurrentChr;
        ...
        if (StealCharacter(CommandBuf) != '.') {
          ...
          CommandBuf->CurrentChr = InitialCharacter+Chrs;
    ...
    case '%': { //% - Percent-command-argument block.
      ...
      BufferKeyChr = CommandBuf->CurrentChr;
      ...
      if (BufferKey == ';') { //This is a very short command terminated with a semicolon - like "%w; m" - ensures that the following command is parsed correctly.
        BufferKey = '0';
        CommandBuf->CurrentChr = BufferKeyChr-1; }
      ...
      if ( (s_CommandChr == 'G') &&  (CommandBuf != s_CommandBuf) ) { //%G command from a macro - copy the text block now.
        ...
        Text = CommandBuf->CurrentRec->text + CommandBuf->CurrentChr + 1;
...
int Run_Sequence(struct Seq *Sequence)
  ...
  for (;;) { // Command-key loop. 
    if (s_BombOut) {
      s_Failure = TRUE;
      return FALSE; }
    if (s_CurrentCommand == NULL) { // First command in sequence.
      s_CurrentCommand = NextCommand;
      if (s_CurrentCommand == NULL)
        return TRUE;
      ...
      if (NextCommand == NULL) { // Last command in sequence.
        ...
        if ( (s_TraceMode & Trace_Failures) && s_Failure ) {
    ...
    if ( ! ++s_CommandCounter) { //Reached terminal count.
      if (s_RecoveryMode) //In recovery mode, execution is abruptly terminated.
        RunError("Command sequence aborted by simulated Ctrl+C");
      else {
        Message(s_CommandBuf, "Reached specified command count");
        s_TraceMode = s_DefaultTraceMode; } }

    if (s_TraceMode & ( Trace_AllCommands | Trace_CommandLines | Trace_Failures ) )
      JotTrace(NULL);
    ...
    case '\'': { //' - Macro call - deferred sequence block.
      ...
      s_BacktraceFrame = &ThisBacktraceFrame;
      s_Failure = ! Run_Sequence(DeferredSeq);
    ...
    case '(': { //( - Block.
      ...
      s_Failure = ! Run_Block(FetchBlock((struct PtrArg **)&ThisArg));
    ...
    case 'N':  //N - Note current Record for later abstract.
      ...
      s_NoteCharacter = s_AbstractWholeRecordFlag ? 0 : s_CurrentBuf->CurrentChr;
      break;

    case 'A': { //A - Abstract using note record pointer.
      char BufferKey = FetchIntArg((struct IntArg **)&ThisArg);
      char ModeQualifier = FetchIntArg((struct IntArg **)&ThisArg);  // NULL, +, - or .
      char FillQualifier = FetchIntArg((struct IntArg **)&ThisArg);  // & or *.
      struct Buf *DestBuf;
      ...
      if (s_CurrentBuf->BufferKey == BufferKey) {
        Fail("Abstract source and destination is the same buffer");
        continue; }
      ...
      if (s_AbstractWholeRecordFlag) {
        s_CurrentBuf->CurrentChr = 0;
        s_CurrentBuf->SubstringLength = 0;
        if (s_NoteLine < s_CurrentBuf->LineNumber)
          s_Failure |= AdvanceRecord(s_CurrentBuf, 1);
        else 
          s_NoteLine++; }
      if (s_Failure)
        continue;
        
      Abstract(DestBuf, s_CurrentBuf, s_NoteLine, s_NoteCharacter, FillQualifier);    
    ...
    case 'Q': { //Q - Qualify.
      s_CurrentBuf->SubstringLength = 0; { // Qualify block. 
        ...
        int CurrentChrPtr = s_CurrentBuf->CurrentChr;
        char Chr;
        int Length, Index;
    ...
    case 'V': { //V - Verify text string.
      ...
      int InitialPointer = s_CurrentBuf->CurrentChr;
      
      s_CurrentBuf->SubstringLength = 0; 
      if (0 < Direction) { // Forward verify.
        if (SearchRecord(s_CurrentBuf, StringArgument, 1))
          LocalFail = ((s_CurrentBuf->CurrentChr) != InitialPointer); }
      else { // Backwards verify. 
        int StringArgLength = strlen(StringArgument);
        s_CurrentBuf->CurrentChr = InitialPointer-StringArgLength;
        if (SearchRecord(s_CurrentBuf, StringArgument, -1))
          LocalFail = (s_CurrentBuf->CurrentChr) != (InitialPointer-StringArgLength);
        s_CurrentBuf->SubstringLength = -StringArgLength; }
      s_CurrentBuf->CurrentChr = InitialPointer;
    ...
    case 'Y':  //Y - Move in same column.
      ...
      if (s_CommandChr == 'Y' && Arg2 == INT_MAX) {
        s_Y_ColumnNo = JotStrlenBytes(s_CurrentBuf->CurrentRec->text, s_CurrentBuf->CurrentChr);
        break; }
      if (Arg1 == '=')
      ...
      if (s_CommandChr == 'Y' && TabStops) { //Identify the TabStop in the original line.
        ...
        TabCount = 0;
        while (SubString-s_CurrentBuf->CurrentRec->text <= s_CurrentBuf->CurrentChr)   {
          ...
          if ( ! (SubString = Pos+1))
            break;
          TabCount++; } }
      s_Failure |= AdvanceRecord(s_CurrentBuf, Arg2);
      ...
      if (s_Failure && (Arg2 != INT_MIN) && (Arg2 != INT_MAX))
        continue;
      break; }
    ...
    case 'B': { //B - Break line.
      ...
      for (Index = 1; Index <= Count; Index +=1) { // Record-loop. 
        ...
        if (Direction < 0) { // Backwards.
          ...
          s_CurrentBuf->CurrentChr = strlen(s_CurrentBuf->CurrentRec->text);
    ...
    case 'T':  //T - Trace i.e. debug
      ...
      s_CommandMode = 0;
      if (s_JournalHand) {
        if (s_TraceMode & Trace_AnyTracePoint ) {
          char Temp[20];
    ...
    case 'F': { //F - Find Command.
      ...
      for ( ; (0 < Count)  && ( ! s_Failure); Count--) {
        for ( ; ; ) { // Record loop, if not found move record pointer.
          ...
          if (SearchRecord(s_CurrentBuf, StringArgument, Direction)) // Substring found.
            ...
            s_Failure = TRUE;
            break; }
          if (s_Failure |= AdvanceRecord(s_CurrentBuf, Direction))
            break;
          if (Direction < 0)
            s_CurrentBuf->CurrentChr = strlen(s_CurrentBuf->CurrentRec->text); } }
      ...
      if (s_Failure) {
        if (0 < SearchSpan)
          s_CurrentBuf->CurrentChr = Direction<0 ? 0 : strlen(s_CurrentBuf->CurrentRec->text);
        continue; }
      if (Traverse) {
        s_CurrentBuf->CurrentChr = (s_CurrentBuf->CurrentChr) + (s_CurrentBuf->SubstringLength);
    ...
    case 'H': { //H - Here command.
      char BufferKey = FetchIntArg((struct IntArg **)&ThisArg);
      long long OverlayMode = FetchIntArg((struct IntArg **)&ThisArg);
      long long Count = FetchIntArg((struct IntArg **)&ThisArg);
      int OriginalCurrentChr = s_CurrentBuf->CurrentChr;
      ...
      for (Index = 1; Index <= Count; Index +=1) { // Copy-buffer loop. 
        if (OverlayMode) { // Overwrite existing records. 
          for ( ; ; ) { // Copy loop.
            int TextLength = strlen(s_CurrentBuf->CurrentRec->text);
            ...
            if (OriginalCurrentChr+SubstringLength <= TextLength)                          //Easy case - whole of new substring is within the original text string.
              ...
              s_CurrentBuf->SubstringLength = TextLength-OriginalCurrentChr;
                ...
                char *NewText = (char *)JotMalloc(NewTextLength+1);
                strcpy(NewText, s_CurrentBuf->CurrentRec->text);
                memset(NewText+TextLength, ' ', NewTextLength-TextLength);
                NewText[NewTextLength] = '\0';
                s_CurrentBuf->CurrentRec->length = NewTextLength;
                free(s_CurrentBuf->CurrentRec->text);
                s_CurrentBuf->CurrentRec->text = NewText; 
                s_CurrentBuf->SubstringLength = SubstringLength; }
            s_CurrentBuf->CurrentChr = OriginalCurrentChr;
            SubstituteString(s_CurrentBuf, SourceBuf->CurrentRec->text, -1);
            if (AdvanceRecord(SourceBuf, 1))
              break;
            if (AdvanceRecord(s_CurrentBuf, 1)) {
              int NewTextLen = OriginalCurrentChr+strlen(SourceBuf->CurrentRec->text)+1;
              GetRecord(s_CurrentBuf, NewTextLen);
              memset(s_CurrentBuf->CurrentRec->text, ' ', NewTextLen);
              s_CurrentBuf->CurrentRec->text[NewTextLen] = '\0'; } }
          ...
          for (;;) { // Record loop. 
            s_Failure |= AdvanceRecord(SourceBuf, 1);
            if (s_Failure)
              break;
            DuplicateRecord(s_CurrentBuf, SourceBuf); }
    ...
    case 'S': { //S - Substitute command.
      ...
      char *StringArgument = (char *)FetchStringArg((struct PtrArg **)&ThisArg, s_InsertString);
      ...
      if ((s_CurrentBuf->SubstringLength) == 0)
        ...
        SubstituteString(s_CurrentBuf, StringArgument, -1);
        if (0 < Arg1) {
          s_CurrentBuf->SubstringLength = -s_CurrentBuf->SubstringLength;
          s_CurrentBuf->CurrentChr = (s_CurrentBuf->CurrentChr) - (s_CurrentBuf->SubstringLength); } }
      break; }
    ...
    case 'I': { //I - Insert command.
      ...
      if (0 < Direction) {
        s_CurrentBuf->SubstringLength = -strlen(StringArgument);
        s_CurrentBuf->CurrentChr = (s_CurrentBuf->CurrentChr) - (s_CurrentBuf->SubstringLength); }
      else
        s_CurrentBuf->SubstringLength = strlen(StringArgument);
      if (s_Failure)
    ...
    case 'E': { //E - Erase bytes or characters.
      ...
      if (Direction < 0) { // -ve. - Erase left Count charcaters. 
        int Current = s_CurrentBuf->CurrentChr;
        
        if (Count == INT_MAX)
          Count = Current;
        else { //Delete a finite no. of characters left of current character.
          int CurrentChr = s_CurrentBuf->CurrentChr;
          if (ShiftRight(s_CurrentBuf, -Count))
            s_Failure = TRUE;
          else
            Count = CurrentChr-s_CurrentBuf->CurrentChr;
          s_CurrentBuf->CurrentChr = CurrentChr; }
        if (Current < Count)
          continue; 
        s_CurrentBuf->CurrentChr -= Count; }
      else { // +ve. - Erase right. 
        ...
        if (Count == INT_MAX)
          ...
          if (ShiftRight(s_CurrentBuf, Count))
            ...
            Count = s_CurrentBuf->CurrentChr-CurrentChr;
          s_CurrentBuf->CurrentChr = CurrentChr; }
    ...
    case 'R': { //R - move current character pointer Right characters.
      ...
      if (Displacement == INT_MAX)
        ...
        s_Failure = ShiftRight(s_CurrentBuf, Displacement);
      if (s_Failure)
        continue;
      break; }
    ...
    case 'K': { //K - delete (Kill) some lines.
      ...
      for (Index = 1; ! s_Failure && (Index <= Count); Index+=1) { // Record Loop. 
        ...
        if (FirstRec && LastRec) {
          ...
          FreeRecord(s_CurrentBuf, (Direction == 1) ? AdjustForwards : AdjustBack);
        if (s_Failure)
          break;
        s_CurrentBuf->UnchangedStatus = 0;
        if (Direction < 0) {
    ...
    case 'P': { //P - Print line(s).
      ...
      for (Index = 1; Index <= Count; Index++) { // Print loop. 
        if (Index == 1) { //First line.
          ...
          s_Failure |= AdvanceRecord(s_CurrentBuf, Direction);
          if (s_Failure)
            break;
    ...
    case 'O': { //O - Stack Operation.
      ...
      case '@': { //O@ reset stack
        long long Seed = (0 < s_StackPtr) && ((s_Stack[s_StackPtr-1].type == FrameType_Int)) ? PopInt() : 1;
        srand(Seed);
      ...
      case 'W': { //OW Scrolls up by lines set in top of stack.
        ...
        while (Win ) 
          if (Win->WindowKey == s_CurrentBuf->BufferKey || Win->WindowKey == '\0')
            break;
          else
            Win = Win->next;
        if ( ! Win ) {
          Fail("Failed to find current buffer in display.");
          continue; }
      ...
      case 'L':{  //OL Literal number to be pushed onto stack.
        ...
        if (Type == 0)
          PushInt(FetchIntArg((struct IntArg **)&ThisArg));
        else
          PushFloat(FetchFloatArg((struct FloatArg **)&ThisArg));
        if (s_Failure)
          continue;
        break; }
      ...
      case '=': { //O= Fail if intergerized top two items on stack unequal.
        ...
        if ((s_Stack[s_StackPtr-1].type == FrameType_Buf) || (s_Stack[s_StackPtr-2].type == FrameType_Buf)) {
          ...
          continue; }
        TopValue = (s_Stack[s_StackPtr-1].type == FrameType_Int) ? PopInt() : (int)PopFloat();
      ...
      case '<': { //O< Test that item at top greater than (top-1).
        ...
        if ( (s_Stack[s_StackPtr-1].type == FrameType_Int) && (s_Stack[s_StackPtr-2].type == FrameType_Int) ) { //Both items are integer - no conversion required.
          ...
          if (OldTopValue > s_Stack[s_StackPtr-1].Value)
            break; 
          continue; }
          ...
          double OldTopValue = (s_Stack[s_StackPtr-1].type == FrameType_Int) ? (double)PopInt() : PopFloat();
      ...
      case '>': { //O> Test that item at top less than (top-1).
        ...
        if ( (s_Stack[s_StackPtr-1].type == FrameType_Int) && (s_Stack[s_StackPtr-2].type == FrameType_Int) ) { //Both items are integer - no conversion required.
          long long OldTopValue = PopInt();
          if (OldTopValue < (long long)s_Stack[s_StackPtr-1].Value)
            break; 
          continue; }
        else if ((s_Stack[s_StackPtr-1].type == FrameType_Buf) || (s_Stack[s_StackPtr-2].type == FrameType_Buf)) {
          Fail("Buffer types not allowed in arithmetic operations.");
      ...
      case 'I': { //OI formatted Input - Convert chr(s) at CurentChr using format key.
        ...
        if (FormatChr == 'C') { //OIC
          if (strlen(s_CurrentBuf->CurrentRec->text) <= s_CurrentBuf->CurrentChr)
            continue;
          PushInt(ImmediateCharacter(s_CurrentBuf));
          s_CurrentBuf->SubstringLength = -1; }
        else if (FormatChr == 'D') { //OID
          long long IntValue;
          if (sscanf((s_CurrentBuf->CurrentRec->text)+(s_CurrentBuf->CurrentChr), "%lld%n", &IntValue, &Length) <= 0)
            continue;
          else {
            PushInt(IntValue);
            s_CurrentBuf->CurrentChr += Length;
            s_CurrentBuf->SubstringLength = -Length;
            break; } }
        else if (FormatChr == 'X') { //OIX
          long long IntValue;
          if (sscanf((s_CurrentBuf->CurrentRec->text)+(s_CurrentBuf->CurrentChr), "%llX%n", &IntValue, &Length) <= 0)
            continue;
          else {
            PushInt(IntValue);
            s_CurrentBuf->CurrentChr += Length;
          ...
          long long IntValue;
          if (sscanf((s_CurrentBuf->CurrentRec->text)+(s_CurrentBuf->CurrentChr), "%llo%n", &IntValue, &Length) <= 0)
            continue;
          else {
            PushInt(IntValue);
            s_CurrentBuf->CurrentChr += Length;
          ...
          float Float;
          if (sscanf((s_CurrentBuf->CurrentRec->text)+(s_CurrentBuf->CurrentChr), "%f%n", &Float, &Length) <= 0) {
            PushFloat(1.0);
          ...
          char *EndPtr;
          float Float = strtof((s_CurrentBuf->CurrentRec->text)+(s_CurrentBuf->CurrentChr), &EndPtr);
          errno = 0;
      ...
      case 'V': { //OV define Value of hashtable data from top of stack.
        ...
        if (BufAtTop) {
          ...
          if (CheckCircularHier(BufFromStack)) {
            Fail("That assignment would result in a circular hierarchy;");
            continue; }
          if (BufFromStack->ExistsInData) {
            Fail("That buffer is alreay used to define a dataObj buffer.");
            continue; }
          if (BufFromStack->ParentObj) {
            Fail("The buffer from stack is already a member of a data-object tree");
            continue; } }
        if ( ! ParentObj->Frame)
          ...
          if (OldItem->type != TopItem->type) {
            Fail("Type mismatch, the data object and the top item on the stack do not match.");
            continue; }
            ...
            if (OrigBuffer != BufFromStack) {
              ...
              if (s_CurrentBuf != OrigBuffer) {
                if (FreeBuffer(OrigBuffer)) {
                  Fail("Attempt to free a read-only or locked buffer.");
                  continue; } } } } }
        if (BufAtTop) {
      ...
      case 'Q': { //OQ Query hashtable, copy previously-defined stack frame to top of stack.
        ...
        if ( ! ThisDataObj->Frame) {
          Fail("Hash-table entry has been defined but data-value not yet assigned.");
          continue; }
      ...
      case 'O': { //OO Output to current text, using given string as format spec.
        ...
        if (s_Stack[s_StackPtr-1].type == FrameType_Buf) {
          ...
          sprintf(TempString, StringArgument, ((struct floatFrame *)(&s_Stack[s_StackPtr-1]))->fValue);
        else //An integer.
          sprintf(TempString, StringArgument, s_Stack[s_StackPtr-1].Value);
        KillTop();
      ...
      case '%': { //O% Remainder
        ...
        if ((s_Stack[s_StackPtr-1].type == FrameType_Int) && (s_Stack[s_StackPtr-2].type == FrameType_Int)) {
          ...
          long long Arg2 = (s_Stack[s_StackPtr-1].type == FrameType_Float) ? (int)PopFloat() : PopInt();
          long long Arg1 = (s_Stack[s_StackPtr-1].type == FrameType_Float) ? (int)PopFloat() : PopInt();
          if (Arg2 == 0) { // Divide by zero.
            s_StackPtr = s_StackPtr+2;
            continue; }
          PushInt(Arg1%Arg2);
          ...
          double Arg2 = (s_Stack[s_StackPtr-1].type == FrameType_Int) ? (double)PopInt() : PopFloat();
          double Arg1 = (s_Stack[s_StackPtr-1].type == FrameType_Int) ? (double)PopInt() : PopFloat();
          switch (Chr) {//Two-operand-stack-operations switch block.
            case '+':
              PushFloat(Arg1+Arg2);
              break;
      ...
      case '.': { //O. Set current line number to <top of stack>.
        int NewLineNo = (int)PopInt();
        int Offset = s_CurrentBuf->LineNumber - NewLineNo;
        if (s_Failure)
          continue;
      ...
      case 'B':  //OB Push current buffer key onto stack.
        PushInt(s_CurrentBuf->BufferKey);
        if (s_Failure)
          continue;
        break;
      ...
      case 'E': { //OE Push Extent (i.e. no. of substring bytes then characters) taking unicode characters into account.
        ...
        if (s_CurrentBuf->NoUnicode) //Unicode is disabled - just duplicate the byte-count entry.
          ...
          if (s_CurrentBuf->SubstringLength) { //Length of substring in display characters.
            if (s_CurrentBuf->SubstringLength < 0)
              PushInt( - JotStrlenBytes(s_CurrentBuf->CurrentRec->text+s_CurrentBuf->CurrentChr+s_CurrentBuf->SubstringLength, -s_CurrentBuf->SubstringLength));
            else 
              PushInt(JotStrlenBytes(s_CurrentBuf->CurrentRec->text+s_CurrentBuf->CurrentChr, s_CurrentBuf->SubstringLength)); }
          else //Length of entire record in display characters.
            PushInt(JotStrlenBytes(s_CurrentBuf->CurrentRec->text, strlen(s_CurrentBuf->CurrentRec->text))); }
        break; }
       
      case 'F': { //OF Push line no. of first line in window.
        PushInt(s_CurrentBuf->OldFirstLineNo);
        break; }
      ...
      case 'P': { //OP push mouse Position - last detected mouse-click position
        ...
        PushInt(s_Mouse_x + (MouseBuf ? MouseBuf : s_CurrentBuf)->LeftOffset);
        PushInt(s_MouseLine);
        PushInt(s_MouseBufKey);
        break; }
       
      case 'N':  //ON Push current line no. onto stack.
        PushInt(s_CurrentBuf->LineNumber);
        if (s_Failure)
          continue;
        break;
      ...
      case 'U': //OU set currently-selected sUbstring length from value on stack.
        if (s_CurrentBuf->NoUnicode) {
          int SubstringLength = PopInt();
          int Len = strlen(s_CurrentBuf->CurrentRec->text);
          if ( (s_CurrentBuf->CurrentChr + SubstringLength) < 0 )
             SubstringLength = -s_CurrentBuf->CurrentChr;
           else if (Len < (s_CurrentBuf->CurrentChr + SubstringLength) )
             SubstringLength = Len - s_CurrentBuf->CurrentChr;
           s_CurrentBuf->SubstringLength = SubstringLength; }
        else {
          int Chrs = PopInt();
          int CurrentChr = s_CurrentBuf->CurrentChr;
          ShiftRight(s_CurrentBuf, Chrs);
          s_CurrentBuf->SubstringLength = s_CurrentBuf->CurrentChr-CurrentChr;
          s_CurrentBuf->CurrentChr = CurrentChr; }
        if (s_Failure)
          continue;
        break;
      ...
      case 'C':  //OC Push current chr. no. onto stack.
        PushInt(s_CurrentBuf->NoUnicode ? s_CurrentBuf->CurrentChr : JotStrlenBytes(s_CurrentBuf->CurrentRec->text, s_CurrentBuf->CurrentChr));
        if (s_Failure)
          continue;
        break;
      ...
      case 'R': //OR shift Right (left if -ve) no. of bytes popped off stack.
        if (s_StackPtr < 1) {
          Fail("Stack underflow");
          continue; }
        s_Failure = ShiftRight(s_CurrentBuf, PopInt());
        if (s_Failure)
          continue;
        break;
      ...
      case 'M':  //OM Move (backwards if -ve) no. of lines popped off stack.
        if (s_StackPtr < 1) {
          Fail("Stack underflow");
          continue; }
        if (AdvanceRecord(s_CurrentBuf, PopInt()))
          continue;
        break;
    ...
    case '%': { //% - Percent-commands block.
      ...
      case 'O': { //%O - Write buffer to file.
        ...
        if (s_RecoveryMode) {
          ...
          if ( ! File) {
            Fail("Failed to open file \"%s\" for writing.", ResolvedPathName);
            continue; }
          if (s_Verbose & Verbose_NonSilent) {
          ...
          while ( ! s_Failure) { // Write out records loop. 
            if (Record == LastRec) {
      ...
      case 'R': { //%R - Run a command file.
        ...
        SwitchToComFile(ScriptPath, asConsole, FirstBlank ? FirstBlank : "");
        if (s_Failure)
          continue;
        break; }
      ...
      case 'I': { //%I - secondary input.
        ...
        int LineNumber, CurrentChr = -1, SubstringLength;
        struct Buf *DestBuf;
        int FileDesc = 0;
        ...
        if (SectionQual || FSectionQual) {
          if (SeekQual || BytesQual || BlockQual || RecordsQual) {
            SynError(s_CommandBuf, "%%%%I -section cannot be used with -seek, -bytes, -records or -block");
            continue; }
          if (FSectionQual) {
        ...
        if (SectionKey) {
          ...
          if ( ! (Object = QuerySection(SectionKey, s_CurrentBuf)) ) {
            Fail("Missing or invalid entry for key \"%s\"", SectionKey);
            continue; }
        ...
        if (FSectionKey) {
          ...
          if ( ! (Object = QueryFSection(FSectionKey, s_CurrentBuf)) ) {
            Fail("Missing or invalid entry for key \"%s\"", FSectionKey);
            continue; }
        ...
        if ( ! BlockQual) {
          if (AppendQual || InsertQual) {
            if (CurrentChr < 0) { //This signifies that the buffer was originally undefined - but with a null record, remove that now unless it's the only record.
              ...
              s_CurrentBuf->CurrentRec = s_CurrentBuf->FirstRec;
              FreeRecord(s_CurrentBuf, DeleteNotAdjust); }
            else if (AppendQual && OrigCurrentRec ==  s_CurrentBuf->FirstRec && CurrentChr == 0) { //Was at start of buffer.
            ...
            for ( ; ; ) { // Insert records loop. 
              ...
              if ((strlen(Record) == 1) && (Record[0] == ':')) { // Last record. 
                if (DestBuf->FirstRec == DestBuf->FirstRec->next)
                  ...
                  FreeRecord(DestBuf, AdjustForwards);
                break; }
              if (s_Failure)
                break; }
            ResetBuffer(DestBuf); }
      ...
      case 'E': { //%E - Execute following CLI command line.
        ...
        if (BufferKey != '\0') {
          ...
          struct Buf *DestBuf;
          FILE *File = NULL;
          char *GivenCommands = (char *)JotMalloc(strlen(RightArg)+1), *ActualCommands = GivenCommands;
          ...
          if (s_RecoveryMode) {
            char RecoveryPathName[StringMaxChr];
            Prompt("Enter recovery-substitution file for %s", GivenCommands);
            do
            ...
            if (GivenCommands[0] == '|') {
              ActualCommands += 1;
              StreamInBuf = TRUE; }
            else if (GivenCommands[0] == '&') {
              if (s_JournalHand) {
                Fail("Asynchronous I/O not available  with journal files.");
                continue; }
              ActualCommands += 1;
              Async = TRUE;
              ...
              if (DestBuf->IOBlock) {
                if (DestBuf->IOBlock->OutPipe) {
                  Fail("Buffer ( %c ) already has asynchronous I/O set up.", DestBuf->BufferKey);
                  continue; } }
              else {
            ...
            if (StreamInBuf)
              ...
              Fail("Could not open reply stream");
              continue; }
#endif
            }
      ...
      case 'F': { //%F - Regular-expression search.
        ...
        if (BufferKey == s_CurrentBuf->BufferKey) {
          SynError(s_CommandBuf, "Destination buffer for %%F is current buffer");
          continue; }
        if (s_TempBuf->CurrentRec->text[0] == '\0') {
          Fail("Empty search expression in %%F.");
          continue; }
        if (s_TempBuf->CurrentRec->text[0] != '\0') {
        ...
        for ( ; ; ) { // Record loop, if not found move record pointer.
          int InitialPointer = s_CurrentBuf->CurrentChr;
          if (SearchRecord(s_CurrentBuf, s_RegExString, Direction)) // Substring found.
            break;
          if (s_Failure |= AdvanceRecord(s_CurrentBuf, Direction))
            break;
          if (Direction < 0)
      ...
      case 'U':  //%U - Undo last substitution.
        ...
        SubstituteString(s_CurrentBuf, s_SearchString, -1);
        break;
      ...
      case 'L': { //%L - set line Length.
        ...
        if (RightArg[0]) { //Term-size expression follows.
          ExpandDeferredString(s_TempBuf, RightArg, TextAsIs);
          sscanf(s_TempBuf->CurrentRec->text + s_TempBuf->CurrentChr, "%dx%d", &GivenWidth, &GivenHeight);
          if (s_TermWidth < GivenWidth || s_TermHeight < GivenHeight) {
            Fail("Height and/orwidth given in %%L expression exceedes corresponding terminal dimension - reverting to %dx%d",
      ...
      case '~':  //%~ - Insert or display control character.
        if (strlen(RightArg) != 0) { // Insert a byte. 
          ...
          SubstituteString(s_CurrentBuf, TempString, -1);
          break; }
        Message(NULL, "\"%X\"", (int)(s_CurrentBuf->CurrentRec->text)[s_CurrentBuf->CurrentChr]);
        break;

      case 'W': //%W - Set up a screen window.
        ExpandDeferredString(s_TempBuf, RightArg, TextAsIs);
        ChangeWindows(BufferKey, s_TempBuf);
        break;
     ...
     case 'S': { //%S - System settings and utilities.
        ...
        if (strstr(Qualifier, "commandmode") == Qualifier) { //%s=commandmode - controls command/type-into-screen mode.
          ...
          if (Value != 0) {
            ...
            if (stat(s_JournalPath, &Stat) ) {
              RunError("Journal directory \"%s\" does not exist, continuing without journal.", s_JournalPath);
              free(s_JournalPath);
              s_JournalPath = NULL; }
            else {
          ...
          char *Temp;
          int ToChrNo = s_AbstractWholeRecordFlag ? strlen(s_CurrentBuf->CurrentRec->text) : s_CurrentBuf->CurrentChr;
          ...
          if ( (ToLineNo < FromLineNo) || ((ToLineNo == FromLineNo) && (ToChrNo < FromChrNo)) ) { //Abstracting backwards - switch around.
            ...
            s_Failure |= AdvanceRecord(s_CurrentBuf, FromLineNo-ToLineNo); }
          if ( ! s_NoteBuffer) {
            continue; }
          if (s_NoteBuffer != s_CurrentBuf) {
            RunError("The note point is in a different buffer");
          ...
          AdvanceRecord(s_CurrentBuf, FromLineNo-ToLineNo);
          s_CurrentBuf->CurrentChr = FromChrNo;
          while (s_CurrentBuf->LineNumber != ToLineNo) {
            char *Line = s_CurrentBuf->CurrentRec->text + s_CurrentBuf->CurrentChr;
            strcpy(Temp+Chr, Line);
            Chr += strlen(Line);
            Temp[Chr++] = '\n';
            AdvanceRecord(s_CurrentBuf, 1); }
          strncpy(Temp+Chr, s_CurrentBuf->CurrentRec->text+s_CurrentBuf->CurrentChr, ToChrNo-s_CurrentBuf->CurrentChr);
          Chr += ToChrNo-s_CurrentBuf->CurrentChr;
          Temp[Chr++] = '\0';
          s_NoteBuffer = NULL;
        ...
        else if (strstr(Qualifier, "commandstring") == Qualifier) { //%s= <string> - return modified string back to command-line input.
          if (Args[0] == ' ')
            Args++;
          ...
          s_Mouse_x = s_CurrentBuf->CurrentChr - s_CurrentBuf->LeftOffset;
          s_Mouse_y = -1;
          ...
          SynError(s_CommandBuf, "Invalid %%%%s command qualifier \"%s\"", Qualifier);
          continue; }
      ...
      case 'P': { //%P - Pipe text (pipe set up by %E with & modifier).
        ...
        if ( ! DestBuf->IOBlock || ! DestBuf->IOBlock->InPipe) {
          Fail("Buffer ( %c ) is not set up for asynchronous I/O.");
          continue; }
      ...
      case 'B': { //%B - Buffer attributes and operations.
        ...
        if (strstr(Qualifier, "unrestricted") == Qualifier) { //As it says - no restrictions.
          ...
          if ( ! TextString && ! ThisColourObj) {
            ...
            ThisTag->StartPoint = s_CurrentBuf->CurrentChr+s_CurrentBuf->SubstringLength;
            ThisTag->EndPoint = s_CurrentBuf->CurrentChr; }
          else {
            ThisTag->StartPoint = s_CurrentBuf->CurrentChr;
            ThisTag->EndPoint = (0<s_CurrentBuf->SubstringLength) ? s_CurrentBuf->CurrentChr+s_CurrentBuf->SubstringLength-1 : strlen(s_CurrentBuf->CurrentRec->text); }
          ThisTag->next = NULL;
          ...
          if (s_CurrentBuf->SubstringLength < 0) {
            TagStartPoint = s_CurrentBuf->CurrentChr+1;
            TagEndPoint = s_CurrentBuf->CurrentChr-s_CurrentBuf->SubstringLength-1; }
          else {
            TagStartPoint = s_CurrentBuf->CurrentChr;
            TagEndPoint = (0<s_CurrentBuf->SubstringLength) ? s_CurrentBuf->CurrentChr+s_CurrentBuf->SubstringLength-1 :  strlen(s_CurrentBuf->CurrentRec->text); }
      ...
      case 'H': { //%H - Hashtable maintenance.
        ...
        if (DoHash(RightArg, BufferKey, Operation, Qual, TextAsIs))
          continue;
        break; }
      ...
      case 'Q': { //%Q - Query.
        ...
        if (QuerySomething(RightArg, BufferKey, Query1, Query2, TextAsIs))
          continue;
    ...
    if (ThisArg != NULL) {
      ...
      return FALSE; }
    LocalFail = FALSE;
    } // Command-key loop ends - destination-point for all continues - from failing commands.
  return ! LocalFail; }
...
void ExitNormally(char *ExitMessage)
  ...
  if (File == NULL) { // Denied write access.
    ...
    for (;!s_Failure;) { // Write out records loop. 
      if (Record == LastRec) {
        if(0 < strlen(Record->text))
          fprintf(File, "%s", Record->text);
        break; }
...
//---------------------------------------------TransCtrlChrs
char * TransCtrlChrs(char * string, int AllowUnicode)
  { //Replaces all control characters with tildes, if AllowUnicode is true, then any negative chr is deemed to be unicode and allowed through.
  int i, Len = strlen(string);
  
  if (AllowUnicode)
    for (i=0; i < Len; ) {
      char Chr = string[i];
      string[i++] = (0 < Chr && Chr < 32) ? '~' : Chr; }
  else
    for (i=0; i < Len; ) {
      char Chr = string[i];
      string[i++] = Chr < 32 ? '~' : Chr; }
...
void DumpStack(struct Buf * DestBuf)
  ...
  for (SP = s_StackPtr-1; 0 <= SP; SP--) { // Stack-dump loop.
    ...
    if (DestBuf) {
      ...
      Message(NULL, "%s", TransCtrlChrs(Line, FALSE)); } }
...
int FreeBuffer(struct Buf *TextBuf)
  ...
  if (TextBuf->ExistsInData || SearchStackForBuf(TextBuf))
    //This usually happens when buffer's been cloned to a DataObject - hence the FreeBuffer operation is silently abandonded.
    return 1;
...
int CheckChildBuffers(struct Buf *ThisBuf)
  ...
  if (ThisBuf == s_CurrentBuf) {
    char * ParentPath = GetBufferPath(ThisBuf);
...
void CopyBuffer(struct Buf *ToBuf, struct Buf *FromBuf)
  ...
  ToBuf->CurrentChr = FromBuf->CurrentChr;
...
//-------------------------------------------------Abstract
void Abstract(struct Buf *DestBuf, struct Buf *SourceBuf, int StartLine, int StartChr, char FillQualifier)
  ...
  int EndChr = SourceBuf->CurrentChr;
  int SourceLim = strlen(SourceBuf->CurrentRec->text);
  int Line;
  int CopyLen;
  
  SourceBuf->SubstringLength = 0;
  if ( (StartLine < EndLine) || ((StartLine == EndLine) && (StartChr < EndChr)) ) { //Abstracting forwards.
    EndLine = StartLine;
    EndChr = StartChr; 
    StartLine = SourceBuf->LineNumber;
    StartChr = SourceBuf->CurrentChr;
    s_Failure |= AdvanceRecord(SourceBuf, EndLine-StartLine);
    if ( (EndLine == StartLine) && (strlen(SourceBuf->CurrentRec->text) < StartChr) )
      Fail("Invalid note point."); }
  
  CopyLen = (EndLine == StartLine) ? StartChr-EndChr : strlen(SourceBuf->CurrentRec->text)-EndChr;
  if (s_Failure)
    return;
  if (SourceLim < StartChr) { //This should never happen ... but see mt2/test308200938.jot_boiled
    Fail("Invalid note point.");
    return; }
  
  if (strlen(SourceBuf->CurrentRec->text) < EndChr || strlen(SourceBuf->CurrentRec->text) < CopyLen) {
    RunError("Garbled note point");
    return; }
  //Copy the first (sub)record.
  DestBuf->SubstringLength = 0;
  SubstituteString(DestBuf, SourceBuf->CurrentRec->text+EndChr, CopyLen);
  if (FillQualifier == '*') { //Backfill with blanks.
    memset(SourceBuf->CurrentRec->text+EndChr, ' ', CopyLen);
    SourceBuf->CurrentChr = EndChr;
    SourceBuf->CurrentRec->DisplayFlag = Redraw_Line; }
  else if (FillQualifier != '&') { //Destroy.
    int Len = strlen(SourceBuf->CurrentRec->text);
    SourceBuf->CurrentChr = EndChr;
    if (Len < SourceBuf->CurrentChr+CopyLen)
      CopyLen = Len-SourceBuf->CurrentChr;
    SourceBuf->SubstringLength = CopyLen;
    SubstituteString(SourceBuf, "", -1); }
  else if (FillQualifier == '&') {
    SourceBuf->CurrentChr = StartChr; }
  if (EndLine != StartLine) {
    s_Failure |= AdvanceRecord(SourceBuf, 1);
    DestBuf->CurrentChr += CopyLen;
    BreakRecord(DestBuf);
    s_Failure |= AdvanceRecord(DestBuf, -1);
    if (s_Failure)
      return; }
  ...
  if (EndLine != StartLine) { //Copy the final (sub)record.
    ...
    DestBuf->CurrentChr = 0;
    DestBuf->SubstringLength = 0;
    if (strlen(SourceBuf->CurrentRec->text) < StartChr) {
      RunError("Garbled note point");
      return; }
    SubstituteString(DestBuf, SourceBuf->CurrentRec->text, StartChr);
    if (FillQualifier == '*') { //Backfill with blanks.
      memset(SourceBuf->CurrentRec->text, ' ', StartChr);
      SourceBuf->CurrentRec->DisplayFlag = Redraw_Line; }
    else if (FillQualifier != '&') { //Remove the source substring.
      SourceBuf->CurrentChr = 0;
      SourceBuf->SubstringLength = StartChr;
      SubstituteString(SourceBuf, "", -1); }
...
int SearchRecord(struct Buf *TextBuf, char *SearchString, int Direction) {
  ...
  if (SubstringLength < 0)
    FirstByte = TextBuf->CurrentChr + SubstringLength + Direction;
  else if (SubstringLength)
    FirstByte = TextBuf->CurrentChr + Direction;
  else
    FirstByte = TextBuf->CurrentChr;
  RefByte = 0;
  ...
  if ( ! RefChr) { //Search was successful
    ...
    TextBuf->CurrentChr = SubstringStart;
    return TRUE; }
  else {
    TextBuf->CurrentChr = 0;
    TextBuf->SubstringLength = 0;
  ...
  if (0 < Direction) { //Forward search.
    ...
    for (;;) { //  Backwards-search record loop - implemented by repeated forward searches in the same line.
      ...
      for ( ; ; ) {
        ...
        frame += frameSize;
        pass2OrMore = 1; }
      s_Failure |= AdvanceRecord(TextBuf, Direction);
      frame = 0; 
...
//----------------------------------------------SubstituteString
void SubstituteString(struct Buf *DestBuf, const char *SourceString, int Length)
  {
  //
  ...
  while (ThisTag) {
    struct AnyTag *NextTag = ThisTag->next;
    if ( (0 < Offset) && (DestTotalLength+Offset <= ThisTag->StartPoint) )                            //The tag is left pointing into hyperspace.
      ThisTag->StartPoint = DestTotalLength+Offset;
    if (ThisTag && (SubStart+SubLength <= ThisTag->StartPoint) ) {                                    //All of tag is past all of modified substring - adjust start point.
      ThisTag->StartPoint += Offset; }
    if (ThisTag && (ThisTag->type != TagType_Target) && (SubStart+SubLength <= ThisTag->EndPoint) ) { //Only tag end is past the replaced text - adjust end point.
        ThisTag->EndPoint += Offset;
      if (FinalLength < ThisTag->EndPoint)
        ThisTag->EndPoint = FinalLength-1; }
    ThisTag = NextTag; } }
...
int ShiftRight(struct Buf *TextBuf, int Displacement)
  ...
  int FirstByte = TextBuf->CurrentChr;          //Points to start-point byte.
...
#endif
      if (ChrCount != Displacement) {
        TextBuf->CurrentChr = (0 < Displacement) ? TotalBytes : 0;
        Fail = TRUE; }
      else
...
int AddPercentArg(struct Com *CommandBlock, struct Buf *CommandBuf)
  ...
  struct PtrArg *NewArg = (struct PtrArg *)JotMalloc(sizeof(struct PtrArg));
  struct AnyArg *ThisArg = CommandBlock->ArgList;
  char *String = (CommandBuf->CurrentRec->text)+(CommandBuf->CurrentChr), *SubString = String, *RealEnd, *BackSlash;
  ...
  if (RealEnd) { //A -hereEndsThePercentCommand - let all of the string through without trying to interpret quotes or semicolons and without stripping backslashes.
    CommandBuf->CurrentChr += RealEnd-String+28;
    ...
    while ( (BackSlash = memchr(SubString, '\\', RealEnd-SubString)) ) { //There's a single backslash - remove it and then copy any more that follow on immediately after.
      ...
      CommandBuf->CurrentChr += BackSlash-SubString;
      if ( ! BackSlash[0] )
        break;
      SubString = BackSlash; }
      
    CommandBuf->CurrentChr += RealEnd-SubString+1;
    if (BackSlash)
...
int QuerySomething(char *QueryString, char BufferKey, int QueryType, int SubQuery, int TextAsIs)
...
#if ! defined(VC)
    DestBuf->CurrentChr = strlen(VERSION_STRING);
  ...
  case Q_system: { //System settings etc.
    ...
    AddFormattedRecord(FALSE, DestBuf, "          Note character = %4d", s_NoteCharacter);
...
#endif
    AddFormattedRecord(FALSE, DestBuf, "   Table-entry separator = '\\x%02X'", s_TableSeparator);
    ...
    while (ThisBuf != NULL) {
      ...
      if (ThisBuf->ExistsInData && ThisBuf->ParentObj) { //It is possible that the original data object has been deleted.
        char *Path = GetBufferPath(ThisBuf);
        if ( ! DataObjs++)
  ...
  case Q_history: { //history - dumps history to specified buffer.
    struct Rec *CommandRec = s_CommandBuf->FirstRec->next;
    if ( ! DestBuf)
      return 1;
    while (CommandRec != s_CommandBuf->FirstRec) {
        ...
        if (((struct JumpObj *)ThisObj)->TargetRec) {
          ...
          if (((struct JumpObj *)ThisObj)->Target)
            AddFormattedRecord(FALSE, 
              ...
              ((struct JumpObj *)ThisObj)->LineNumber, ((struct JumpObj *)ThisObj)->Target->StartPoint, TruncText);
          else
            AddFormattedRecord(FALSE, 
              DestBuf, 
  ...
  case Q_tags: { //tags - sends a list of record tags in current buffer to nominated buffer.
    ...
    if (SubQuery == Q_here) { //Report only tags active at current character position.
      ...
      AddFormattedRecord(FALSE, DestBuf, "Reporting tags at Line no. %d Character no. %2d of Buffer %c", SourceBuf->LineNumber, SourceBuf->CurrentChr, SourceBuf->BufferKey);
    ...
    while (TRUE) {    
      ...
      while (ThisTag) {
        if ( ! Here || ( (ThisTag->StartPoint <= SourceBuf->CurrentChr) && ( (ThisTag->type == TagType_Target) || (SourceBuf->CurrentChr <= ThisTag->EndPoint) ) ) ) {
          if (ThisTag->type == TagType_Target) {
            ...
            AddFormattedRecord(FALSE, DestBuf, "  Type target %c, Chr %2d, %s", 
              Object->HashBuf->BufferKey, ThisTag->StartPoint, ErrorTag ? "  **Error JumpObj-record mismatch **" : Object->HashKey);
            ErrorTags += ErrorTag;
            ErrorRec |= ErrorTag; }
          else if (ThisTag->type == TagType_Text)
            AddFormattedRecord(FALSE, DestBuf, "  Type text from chr %3d to %3d = \"%s\"", ThisTag->StartPoint, ThisTag->EndPoint, ThisTag->Attr);
          else if (ThisTag->type == TagType_Colour) {
            AddFormattedRecord(FALSE, DestBuf, "  At rec %3d, chr %2d to %2d, active colour is %s",
              LineNo, ThisTag->StartPoint, ThisTag->EndPoint, ((struct ColourObj *)ThisTag->Attr)->TagName); }
          else if (ThisTag->type == TagType_Font)
            AddFormattedRecord(FALSE,  DestBuf, "  Type Font at chr %2d to %2d", ThisTag->StartPoint, ThisTag->EndPoint); }
        ThisTag = ThisTag->next; }
  ...
  case Q_buffer: { //buffer - returns info on buffer.
    ...
    if (SourceBuf->ParentObj) {
      char * ParentPath = GetBufferPath(SourceBuf);
    ...
    AddFormattedRecord(FALSE, DestBuf, "           CurrentChr = %d", SourceBuf->CurrentChr); 
  ...
  case Q_verify: { //Verifies the basic integrity of the buffer and the sequence.
    ...
    if (SourceBuf->CurrentChr < 0) {
      ErrorsDetected++;
      if (DestBuf)
        AddFormattedRecord(FALSE, DestBuf, "Current Character is negative (%d)",  SourceBuf->CurrentChr); }
    if (TotalLen < SourceBuf->CurrentChr || TotalLen < SourceBuf->CurrentChr+SourceBuf->SubstringLength) {
      ErrorsDetected++;
      if (DestBuf) {
        AddFormattedRecord(FALSE, DestBuf, "Overall string length %d, is exceeded by either (CurrentChr=%d) or CurrentChr+(SubstringLength=%d)",
    ...
    do { 
      ...
      if (ThisRec->length < Length) {
        ...
        break; }
      if (DestBuf)
        AddFormattedRecord(FALSE, DestBuf, "Rec %4d:\"%s\"", LineNo++, ThisRec->text);
...
//-----------------------------------------------GetBufferPath
char * GetBufferPath(struct Buf *ThisBuf) {
  //Returns a malloc'ed string giving the correct hashtable hierarchy path for the nominated buffer.
  char * ParentPath;
  struct PathElem {
    struct Buf * Buf;
...
int Backtrace(struct Buf * DestBuf) {
  ...
  while (Command) {
    ...
    if ( ! ThisFrame)
      strcpy(String, "Invalid backtrace stack-frame");
    else {
      int Bytes = 0, Chrs = 0;
      if (ThisFrame->type == ScriptFrame) { //ScriptFrame == 1
      ...
      else if (ThisFrame->type == MacroFrame) { //MacroFrame == 2
        char Key = Command->CommandBuf->BufferKey;
        Chrs = sprintf(String, "  Line %4d of macro %c: ", Command->CommandLineNo, Key ? Key : ' '); }
      else if (ThisFrame->type == CallFrame) {  //CallFrame == 3
        char Key = Command->CommandBuf->BufferKey;
        Chrs = sprintf(String, "  Line %4d of ( %c ) - function %s: ", Command->CommandLineNo, Key ? Key : ' ', ThisFrame->FirstRec->prev->prev->text); }
      else if (ThisFrame->type == ConsoleCommand) { //ConsoleCommand == 4
        char Key = Command->CommandBuf->BufferKey;
        Chrs = sprintf(String, "  Line %4d of %s %c: ", Command->CommandLineNo, Key == 'i' ? "-init commands" : "console command", (Key ? Key : ' ')); }
      else if (ThisFrame->type == DebuggerCommand) {  //DebuggerCommand == 5
        char Key = Command->CommandBuf->BufferKey;
      ...
      if (DestBuf) {
        ...
        ScrollConsole(FALSE, FALSE);
        JotAddString(String, Chrs);
        if ( ! RedefinedMacro ) { //The original source is available.
          int MaxBytes = strlen(Command->CommandRec->text);
          JotAddBoundedString(Command->CommandRec->text, MaxBytes, ComChrNo, &Bytes, &Chrs, 0, Chrs, MaxBytes);
          if (s_TTYMode)
            JotAddBoundedString("[", 1, 1, &Bytes, &Chrs, 0, Chrs, MaxBytes);
          else
            JotSetAttr(Attr_CurrCmd);
          JotAddBoundedString(Command->CommandRec->text+ComChrNo, MaxBytes, ComChrs, &Bytes, &Chrs, 0, Chrs, MaxBytes);
          if (s_TTYMode)
            JotAddBoundedString("]", 1, 1, &Bytes, &Chrs, 0, Chrs, MaxBytes);
          else
            JotSetAttr(Attr_Normal);
          JotAddBoundedString(Command->CommandRec->text+ComChrNo+ComChrs, MaxBytes-ComChrNo-ComChrs, MaxBytes, &Bytes, &Chrs, 0, Chrs, MaxBytes); }
          ...
          JotAddBoundedString(String, strlen(String), StringLimit, &Bytes, &Chrs, 0, 0, StringLimit);
          JotSetAttr(Attr_CurrCmd);
          ChrNo = 0;
          DumpCommand(ThisCom, &ChrNo, String);
          JotAddBoundedString(String, strlen(String), StringLimit, &Bytes, &Chrs, 0, Chrs, StringLimit);
          JotSetAttr(Attr_Normal);
          for (i = 0; i < 10; i++) {
...
void ExpandDeferredString(struct Buf *DestinationBuf, char *Arg, int TextAsIs)
  ...
  while ( ! TextAsIs && (Quote = strchr((Rec = DestinationBuf->CurrentRec->text)+StartPoint, '\''))) {
    ...
    if ( (2 < Quote-Rec && ((Quote-1)[0] == '\\') && ((Quote-2)[0] == '\\') ) ) //It's a string of backslashes - let them through unchanged.
      ...
      DestinationBuf->CurrentChr = Quote-Rec-1;
...
//----------------------------------------------ChangeWindows
void ChangeWindows(char Key, struct Buf *CmdBuf)
  ...
  while (Win) { // Slice-counting loop.
    ...
    if ((Win->WindowType & WinType_Mask) == WinType_Slice) {
      ...
      if (s_TermWidth < Win->Right) {
        Fail("win:%d extends beyond the right screen margin.", WinNo);
        Win->WindowType |= WinType_Void; } }
      else
        Slices = 1;
    if (s_TermHeight < Win->Bot) {
      Fail("win:%d extends below bottom screen margin.", WinNo);
      Win->WindowType |= WinType_Void; }
    WinNo++;
    Win = Win->next; }
...
int GetChrNo(struct Buf *TextBuf)
  ...
  int CurrentByte = TextBuf->CurrentChr;
  int TabCount = 1;
  int TabStop = 0;
  int Bytes = 0;
  int FirstByte = 0;
  ...
  if (TabChrs) { //Display a section of a tab-separated-cells record.
    ...
    for ( ; ByteNo < LastByte && s_ChrNo < TabChrs[TabIndexMax]; ) {
      if (TabIndex < 0 || TabBytes[TabIndex+1]-2 < ByteNo) {            //New cell - setup.
        ...
        if (0 < PaddingChrs) {                                          //Text is shorter than allocated cell - justify with blanks.
          ...
          JotAddBoundedString(Padding, PaddingChrs, SliceWidth, &ByteCount, &ChrCount, ColourPair, Left, SliceWidth);
          s_ChrNo += ChrCount; }
        else if (PaddingChrs < 0) {                                     //Negative padding indicates a truncated cell.
          JotAddBoundedString("!", 1, 1, NULL, NULL, ColourPair, Left, SliceWidth);
      ...
      if (SliceWidth <= EndStopChr-s_ChrNo) //No point in attempting to display anything beyond the right margin.
        return FALSE;
      JotAddBoundedString(String+ByteNo, LastByte-ByteNo, EndStopChr-s_ChrNo, &ByteCount, &ChrCount, ColourPair, Left, SliceWidth);
      ByteNo += ByteCount;
      s_ChrNo += ChrCount;
    ...
    while (s_ChrNo-LeftOffset < SliceWidth && ByteNo < LastByte ) { //Tab-separated substring loop - exits when we reach the right-screen boundary.
      ...
      if (s_ChrNo+Chrs < LeftOffset) {                                                            //None of this substring is viewable - all left of the leftOffset.
        ...
        if (s_ChrNo < LeftOffset) {                                                               //Only part of this substring is in view.
          ...
          JotAddBoundedString(
            String+ByteNo+NextVisibleByte, EndStopByte-ByteNo-NextVisibleByte, SliceWidth-s_ChrNo+LeftOffset, &Bytes, NULL, ColourPair, Left, SliceWidth);
          ByteNo += Bytes+NextVisibleByte; }
        else if (ByteNo < TotalBytes) {                                                           //All of this substring is in view.
          JotAddBoundedString(String+ByteNo, EndStopByte-ByteNo, SliceWidth-s_ChrNo+LeftOffset, &Bytes, NULL, ColourPair, Left, SliceWidth);
          ByteNo += Bytes; }
        else if (ByteNo == TotalBytes && Attr == Attr_CurrChr && s_ChrNo+LeftOffset < SliceWidth)
          JotAddBoundedString("~", 1, 1, NULL, &Chrs, ColourPair, Left, SliceWidth);   //CurrentChr is at the end of the string.
        s_ChrNo += Chrs;
      ...
      if (NextSubstring && NextSubstring[0] == s_TableSeparator) { //Tab-terminated substring - calculate and add correct number of padding blanks.
        ...
        if (LeftOffset <= TabPoint) { //Some of the padding is in the visible area of line.
          ...
          memset(Padding, ' ', PaddingChrs);
          Padding[PaddingChrs] = '\0';
          JotAddBoundedString(Padding, PaddingChrs, SliceWidth-s_ChrNo+LeftOffset, NULL, &Chrs, ColourPair, Left, SliceWidth); }
        s_ChrNo = TabPoint;
        ByteNo++; }
      else
        return LastByte <= ByteNo && InView; }
    ...
    if (FirstByte <= LastByte) {
      if (ByteNo < TotalBytes)
        JotAddBoundedString(String+FirstByte, LastByte-FirstByte, SliceWidth-s_ChrNo+LeftOffset, &Bytes, &Chrs, ColourPair, Left, SliceWidth);
      else if (ByteNo == TotalBytes && Attr == Attr_CurrChr && s_ChrNo+LeftOffset < SliceWidth)
        JotAddBoundedString("~", 1, 1, NULL, &Chrs, ColourPair, Left, SliceWidth);
      s_ChrNo += Chrs;
      ByteNo = LastByte;
...
int JotUpdateWindow()
  ...
  struct Window *Win = s_FirstWindow;
  int DisplayedCurrentChr = FALSE;                    //Flag is set TRUE only when the CurrentChr of the s_CurrentBuf appears in display. 
  ...
  while (Win) { // Window loop.
    ...
    if (TextBuf && ! (Win->WindowType & (WinType_Frozen | WinType_Void))) {
      ...
      int CheckRange = WindowHeight-(LineNumber-OldFirstLineNo)-AllPadding;
      ...
      for (ThisLine = Top; ThisLine <= Top+WindowHeight-AllPadding-1; ThisLine++) { // Record update loop.
        ...
        if (s_TermHeight-s_NewConsoleLines-1 <= ThisLine) {
          break; }
        if( ! Record) { //Reached end of buffer before end of window - just fill in with blank lines.
          if (s_TermTable[SliceIndex+ThisLine]) { // Clearing unused lines loop.
        ...
        if ( ! DisplayedCurrentChr && (Record == TextBuf->CurrentRec) && (TextBuf == s_CurrentBuf) ) { //Current line of current buffer, overlay line with cursor and substring highlights.
          int CurrentChrEndByte = CurrentChr+JotStrlenChrs(Record->text+CurrentChr, 1);
          int EndStop = strlen(Record->text);
          int ColourPair = 0;
          int NextColourPoint, LastStop = -1, NextHighlight;
          int Attr = Attr_Normal;
          
          DisplayedCurrentChr = TRUE;
          while (ThisTag && ThisTag->type != TagType_Colour) 
            ThisTag = ThisTag->next;
          NextColourPoint = (ThisTag ? ThisTag->StartPoint : EndStop);
          ...
          while ( 1 ) { //Colour-tag loop.
            if (LastStop < NextHighlight && NextHighlight <= NextColourPoint) { //Reached the NextHighlight point.
              LastStop = NextHighlight;
              if (SubstringLength < 0 && NextHighlight == CurrentChr+SubstringLength) { //At the start of a negative substring.  
                WriteString(Record->text, ThisLine, Attr_Normal, New, CurrentChr+SubstringLength, LeftOffset, ColourPair, TextBuf->TabStops, TabCells, Win);
                New = FALSE;
                Attr = Attr_SelSubStr;
                NextHighlight = CurrentChr; }
              else if (NextHighlight == CurrentChr) { //At current character (+ve or no substring case), highlight it now.
                WriteString(Record->text, ThisLine, Attr, New, CurrentChr, LeftOffset, ColourPair, TextBuf->TabStops, TabCells, Win);
                s_InView = WriteString(Record->text, ThisLine, Attr_CurrChr, (New = FALSE), CurrentChrEndByte, LeftOffset, ColourPair, TextBuf->TabStops, TabCells, Win);
                if (SubstringLength < 0) {
                ...
                WriteString(Record->text, ThisLine, Attr, New, NextHighlight, LeftOffset, ColourPair, TextBuf->TabStops, TabCells, Win);
                NextHighlight = EndStop;
                Attr = Attr_Normal; } }
            else if (ThisTag) {
              LastStop = NextColourPoint;
              if (NextColourPoint <= ThisTag->EndPoint) { //A new colour tag.
                WriteString(Record->text, ThisLine, Attr, New, ThisTag->StartPoint, LeftOffset, 0, TextBuf->TabStops, TabCells, Win);
                New = FALSE;
                ColourPair = ((struct ColourObj *)ThisTag->Attr)->ColourPair;
                NextColourPoint = ThisTag->EndPoint+1; }
              else { //Tagged text pending from previous iteration.
                WriteString(Record->text, ThisLine, Attr, FALSE, ThisTag->EndPoint+1, LeftOffset, ((struct ColourObj *)ThisTag->Attr)->ColourPair, TextBuf->TabStops, TabCells, Win);
                ColourPair = 0;
                while ( (ThisTag = ThisTag->next) ) 
                  if (ThisTag->type == TagType_Colour)
                    break;
                NextColourPoint = ThisTag ? ThisTag->StartPoint : EndStop; } }
            else { //Current-character highlights and colour tags all done - write to end of line and exit.
              WriteString(Record->text, ThisLine, Attr_Normal, New, EndStop, LeftOffset, ColourPair, TextBuf->TabStops, TabCells, Win);
              JotClrToEndOfSlice(ThisLine, LeftOffset, Left, Right);
              break; } }
          
          s_y = ThisLine;
          s_x = JotStrlenBytes(s_CurrentBuf->CurrentRec->text, s_CurrentBuf->CurrentChr);
          Record->DisplayFlag = Remove_Cursor; }
           
        else if ( (Record->DisplayFlag != Redraw_None) || (s_TermTable[SliceIndex+ThisLine] != OffsetText) ) { //Update screen image of all all non-current records.
          struct ColourObj *ColourObj = NULL;
          int EndStop = strlen(Record->text);
          
          while (ThisTag && ThisTag->type != TagType_Colour) 
            ThisTag = ThisTag->next;
            
          while (ThisTag && (ThisTag->StartPoint) < EndStop) { //Colour-tag loop
            WriteString(Record->text, ThisLine, Attr_Normal, New, ThisTag->StartPoint, LeftOffset, 0, TextBuf->TabStops, TabCells, Win);
            New = FALSE;
            ColourObj = (struct ColourObj *)ThisTag->Attr;
            WriteString(Record->text, ThisLine, Attr_Normal, New, ThisTag->EndPoint+1, LeftOffset, ColourObj->ColourPair, TextBuf->TabStops, TabCells, Win);
            while ( (ThisTag = ThisTag->next) ) 
              if (ThisTag->type == TagType_Colour)
                break; }
          WriteString(Record->text, ThisLine, Attr_Normal, New, EndStop, LeftOffset, 0, TextBuf->TabStops, TabCells, Win);
          if (PopupWindow)
            s_ChrNo += Win->Right - Win->Left - Win->PopupWidth;
          JotClrToEndOfSlice(ThisLine, LeftOffset, Left, Right);
          if ( ! DisplayedCurrentChr)
      ...
      if (SeparatorString && (s_TermTable[SliceIndex+Bot-1] != SeparatorString) ) {
        if (Bot < s_TermHeight-s_NewConsoleLines) {
          ...
          if(strlen(SeparatorString) < MaxNameLength) {
            ...
            JotAddString(SeparatorString, ActualLength); }
          else {
            int SepStart = strlen(SeparatorString)-MaxNameLength;
            ActualLength = MaxNameLength;
            JotAddString("...", 3);
            JotAddBoundedString(SeparatorString+SepStart, SliceWidth, MaxNameLength, NULL, NULL, 0, 0, 0); }
...
int ReadBuffer(FILE *File, int FileDesc, int NonUnicode, char *Name, struct Buf * DestBuf, 
  ...
  if (s_JournalHand) {
    ...
    char RecoveryPathname[StringMaxChr], Temp[StringMaxChr];
        ...
        case MOUSE_EVENT:
          if (s_MouseMask != 0) {
            ...
            while (Win != NULL) { // Window loop - search windows for mouse-event x, y.
              if (Win->Top <= s_Mouse_y) {
                struct Buf *ThisBuf;
                int FirstLineNumber, FirstLineNo;
  ...
  for ( ; ; ) { // Main character loop.
    ...
    case RubOut:
      ...
      if (ScreenMode) {
        ...
        if (0 < ByteCount) { //Easy case - delete characters from CommandBuf.
          JotDeleteChr(s_x = ByteCount+(s_CurrentBuf->TabStops ? GetChrNo(s_CurrentBuf)-1 : s_CurrentBuf->CurrentChr)-LeftOffset-2, s_y);
          ByteCount -= ChrByteCount; }
        else { //Delete character from current buffer.
          if (s_CurrentBuf->CurrentChr <= 0) { //No more characters before cursor in current record first join with predecessor record.
            AdvanceRecord(s_CurrentBuf, -1);
            JoinRecords(s_CurrentBuf); }
          else { //Delete main-buffer characters to the left of the cursor.
            int ChrByteCount = JotBytesInChEndr(s_CurrentBuf->CurrentRec->text, s_CurrentBuf->CurrentChr-1);
            int ChrPtr = s_CurrentBuf->CurrentChr -= ChrByteCount;
      ...
      if ( 32 <= Chr || Chr < 0 || Chr == '\t') //An ordinary printable ASCII character a TAB, or a unicode byte.
        ...
        if (ScreenMode) {
          ...
          if (s_CommandMode & Command_OverTypeMode) {
            int LeftOffset = s_CurrentBuf->LeftOffset;
            JotDeleteChr(s_x = ByteCount+(s_CurrentBuf->TabStops ? GetChrNo(s_CurrentBuf)-1 : s_CurrentBuf->CurrentChr)-LeftOffset, s_y); }
          JotInsertChr(Chr); }
        else if( ! s_TTYMode)
#if defined(VC)
          JotAddChr(Chr);
    ...
    case '\r':
      ...
      if (ScreenMode) { //There are typed-into-screen characters insert them to s_CurrentBuf then exit - deal with the escape sequence next time around.
        ...
        s_CurrentBuf->CurrentChr += ByteCount;
        s_CurrentBuf->LineNumber--;
        s_CurrentBuf->SubstringLength = 0;
        BreakRecord(s_CurrentBuf);
        if (s_JournalHand) {
...
int TransEscapeSequence(struct Buf *DestBuf, FILE *FileHandle, signed char Chr, char *EscapeString)
  ...
  if (ChrPointer && ScreenMode) { //There are typed-into-screen characters insert them to s_CurrentBuf then deal with the escape sequence.
    if (s_CommandMode & Command_OverTypeMode) {
      int RightSubstringLen = strlen(s_CurrentBuf->CurrentRec->text) - s_CurrentBuf->CurrentChr;
    ...
    s_CurrentBuf->CurrentChr += ChrPointer;
    s_CurrentBuf->SubstringLength = 0;
    if (s_JournalHand)
  ...
  for ( ; ; ) { // Translation-search record loop.
    ...
    if (EscapeSequenceLength <= SearchIndex+ModifyCount)
      break;
    if ( (s_Failure = (AdvanceRecord(TranslationBuf, 1))) )
      break; }
    
  if (s_Failure) { // No match.
  ...
  if (asConsole && s_RecoveryMode) //Suck out any trailing whitespace and the '\n' from the recovery file.
    while ( (Chr = JotGetCh(FileHandle) == ' ') )
      { }
  return asConsole ? s_Failure : FALSE; }
...
int SwitchToComFile(char *NameString, char *asConsole, char *Args)
  ...
  if (s_RecoveryMode) { //In recovery mode prompt then read the journal-area pathname.
    ...
    if (s_JournalHand) {
      if ( ! euidaccess(Actual, W_OK)) { //Making a journal copy of a writable file.
    ...
    for (;;) { // Command-file-record loop. 
      ...
      if ( ! status && ! s_BombOut) {
        if (s_EditorInput->asConsole) //Forgive failing lines in -asConsole scripts.
          ...
          RunError("Command-file failure, line %d of \"%s\"", s_EditorInput->LineNo, Actual);
...
int JotAddChr(char Chr)
  ...
  int CurrentChrSave = s_CurrentBuf->CurrentChr;
  int Chrs = 1;
#if defined(VC)
  const CHAR_INFO  fill = { { ' ' }, Attr_Normal };
  ...
  if (Chr < 0) {
    if (s_NoUnicode)
      ...
      JotAddBoundedString(UTF_8_Chr, NextByteOfUTF_8, 1, NULL, &Chrs, 0, 0, 0);
      if (1 <= Chrs) {
        NextByteOfUTF_8 = 0;
        return 1; }
      else
  ...
  refresh();
#endif
  s_CurrentBuf->CurrentChr = CurrentChrSave;
  return Chrs; }

//----------------------------------------------JotAddBoundedString
void JotAddBoundedString(
      char *OrigString, int MaxBytes, int MaxChrs, int *ByteCount, int *ChrCount, int ColourPair, short unsigned int SliceOffset, short unsigned int SliceWidth)
...
//----------------------------------------------JotAddString
void JotAddString(char *String, int MaxChrs)
  { //Replacement for curses addstr() - writes string to screen - typically the console area.
#if defined(VC)
  printf("%s", String);
#elif defined(LINUX)
  if (s_TTYMode)
    ...
    memset(WideString, '\0', MaxChrs+1);
    if ((Len = mbstowcs(WideString, String, MaxChrs)) < 0) {
...
void JotClrToEndOfSlice(int TermLine, int LeftOffset, int Left, int Right)
  ...
  if ( (s_ChrNo-LeftOffset < 0) || (Right-Left <= s_ChrNo-LeftOffset) ) //Exit early if LeftOffset is incorrectly set.
    return;
...
int Fail(char *FormatString, ...)
  ...
  if ( (s_Verbose & Verbose_QuiteChatty) || (s_Verbose & Verbose_PrintLine) ) {
    ...
    if (s_Verbose & Verbose_QuiteChatty)
      DisplaySource(s_CurrentCommand, String);
    if (s_Verbose & Verbose_PrintLine)
      DisplayDiag(s_CurrentBuf, TRUE, ""); }
  s_Failure = TRUE;
  return 1; }
...
void Message(struct Buf *CommandBuf, char *FormatString, ...)
  ...
  JotAddString(TransCtrlChrs(string, TRUE), s_TermWidth);
  JotGotoXY(s_x, s_y); }
...
void JotDebug(struct Buf *CommandBuf, char *FormatString, ...)
  ...
  if (CommandBuf)
    ...
    ScrollConsole(FALSE, FALSE);
    JotAddString(string, s_TermWidth); 
    JotGotoXY(s_x, s_y); } }
...
void DisplaySource(struct Com *Command, char *Message)
  ...
  if ( (Command != NULL) && (MacroStatus & SameSinceCompiled) && ((MacroStatus & MacroRun) == 0) ) { //Commannd buffer has not been redefined.
    ...
    int i, ChrNo;
    struct Com *ThisCom = Command;
    ChrNo = sprintf(String, "  Line %4d, Chr %d of macro %c: (disassembly) ", Command->CommandLineNo, Command->CommandChrNo, Key ? Key : ' ');
...
void DisplayDiag(struct Buf *TextBuf, int ShowCursor, char *MessageText)
  ...
  char *Record;
  int RecordLength, CurrentChr, SubstringLength, SubstringLeft, SubstringRight, FirstByte, CurChrNo, MessageLength, WidthLimit, ChrLimit, Chrs, Bytes, Result;
  
  if ( ! TextBuf->CurrentRec)
    return;
    
  RecordLength = strlen(TextBuf->CurrentRec->text);
  Record = (char *)JotMalloc(RecordLength+2);  //That's one extra byte for the string terminator and one more in case a trailing '~' is required.
  CurrentChr = TextBuf->CurrentChr;
  SubstringLength = TextBuf->SubstringLength;
  ...
  for (CurChrNo = 0; CurChrNo <= RecordLength; CurChrNo++) {
    signed char Chr = TextBuf->CurrentRec->text[CurChrNo];
    if (Chr < ' ') {
      if (Chr < 0) {
        if (s_NoUnicode)
          Chr = '~'; }
  ...
  if (ShowCursor) { // Display cursor.
    ...
    if (WidthLimit < ChrLimit)
      ChrLimit = WidthLimit;
    JotAddBoundedString(Record+FirstByte, SubstringLeft-FirstByte, ChrLimit, NULL, &Chrs, 0, 0, 0);
    Bytes = SubstringLeft;
    ...
    if (SubstringLeft == CurrentChr) { //Substring begins at current chr or no substring
      ...
      if (RecordLength-1 < Bytes && Chrs < WidthLimit) {
        JotAddBoundedString("~", 1, 1, &Result, NULL, 0, 0, 0);
        Bytes += Result-1; }
      else {
        JotAddBoundedString(Record+SubstringLeft, RecordLength-SubstringLeft, 1, &Result, NULL, 0, 0, 0);
        Bytes += Result; }
      Chrs++;
      if (SubstringLeft < SubstringRight) {
        JotSetAttr(Attr_SelSubStr);
        JotAddBoundedString(Record+Bytes, SubstringRight-Bytes+1, ChrLimit-Chrs, NULL, &Result, 0, 0, 0);
        Chrs += Result;
        Bytes = SubstringRight+1; } }
    else { // Current character follows end of substring
      JotSetAttr(Attr_SelSubStr);
      JotAddBoundedString(Record+SubstringLeft, SubstringRight-SubstringLeft, ChrLimit-Chrs, NULL, &Result, 0, 0, 0);
      Chrs += Result;
      if (WidthLimit <= Chrs) {
        free(Record);
        return; }
      ...
      JotSetAttr(Attr_CurrChr);
      JotAddBoundedString(Record+Bytes, RecordLength-Bytes, 1, &Result, NULL, 0, 0, 0);
      Bytes += Result;
      Chrs++;
      if (RecordLength <= Bytes && Chrs < WidthLimit) {
        JotAddBoundedString("~", 1, 1,  NULL, &Result, 0, 0, 0);
        Chrs += Result; } }
    JotSetAttr(Attr_Normal);
    JotAddBoundedString(Record+Bytes, RecordLength-Bytes, ChrLimit-Chrs, NULL, NULL, 0, 0, 0);
    JotGotoXY(0, s_TermHeight-1);
    
    if (s_TTYMode) {
      if (s_Verbose & Verbose_NonSilent) {
        int i = 0, Lim = MessageLength + (SubstringLeft ? SubstringLeft : CurrentChr);
        ScrollConsole(FALSE, TRUE);
        for ( ; i < Lim; i++)
          fputc(' ', stdout);
        if (CurrentChr == SubstringLeft)
          fputc('^', stdout);
        if (SubstringLeft < SubstringRight) {
          for ( ; i < MessageLength+SubstringRight; i++)
            fputc('~', stdout);
          if (SubstringRight && (CurrentChr == SubstringRight))
            fputc('^', stdout); }
        fflush(stdout);
        fputc('\n', stdout); }
      else
    ...
    if (s_TTYMode) // Write line to stdout/terminal.
      fputs(TextBuf->CurrentRec->text, stdout);
    else {
      JotAddString(Record, WidthLimit);
      JotGotoXY(s_x, s_y); } }
...
void JotTrace(char * Message)
  ...
  if (Message)
    ...
    else if ( (s_TraceMode & Trace_CommandLines) && s_CurrentCommand->CommandLineNo != s_LastLineNumber ) {
      s_LastLineNumber = s_CurrentCommand->CommandLineNo;
      strcpy(TraceMessage, "New command line."); }
    else if ( (s_TraceMode & Trace_Failures) && s_Failure )
      strcpy(TraceMessage, "Command failure."); }
    
  if (TraceMessage[0]) {
    int Orig_Verbose = s_Verbose;
...
void JotBreak(char *Text)
  ...
  for ( ; ; ) {
    ...
    DisplaySource(s_CurrentCommand, Text);
...
void Disaster(char *Text, ...)
  ...
  JotTidyUp();
  fprintf(stdout, "\n%s\n\r", string);
  fputs("Backtrace follows:", stdout);
  s_TTYMode = TRUE;
  Backtrace(NULL);
  fputc('\n', stdout);
  exit(1); } 
...
#if defined(VC)
  if (s_OriginalSession && s_JournalHand) { //Close and delete the journal files.
    HANDLE DirHand = INVALID_HANDLE_VALUE;
    WIN32_FIND_DATA FileData;
    DWORD dwRet;
    struct stat Stat;
    ...
    if (NomBuf->FirstObj) { //Restore all non-zombie objects in new hashtable.
      ...
      while (ThisObj) {
        ...
        if ( ! hsearch_r(NewVal, ENTER, &OldVal, NomBuf->htab)) {
          Fail("Failed to restore hashtable item %s - ", ThisObj->HashKey);
          return 1; }
        ThisObj = ThisObj->next; } }
    return ( ! status); }
    ...
    if (Object) {
      ...
      if (Object && (Object->type == ObjType_Jump) ) { //A genuine JumpObj
        ...
        if ( (TargetBuf = Object->TargetBuf) ) {
          ...
          TargetBuf->CurrentChr = Object->Target ? Object->Target->StartPoint : 0;
